    ...
    for model in model_list:
        ...
        df_dedup = (
            df.withColumn("rn", F.row_number().over(w))
              .filter(F.col("rn") == 1)     # keep only rank = 1
              .drop("rn")                   # remove helper column
        )

        # 4. Continue weekly / flag / code logic,
        #    but switch the data source to df_dedup
        ...
        # ----------------------------
        # Flag indicating whether the symptom appears in that week (has_umls = 1)
        ...
        df_flags = (
            df_flags.join(df_other, on=["person_id", "week_start"], how="left")
                    .fillna({"has_any": 0})
        )

        # Create final code: 0 / 1 / 2 encoding
...
# MAGIC %sql
# MAGIC select 'case' as group,count(*),count(distinct person_id) from wei_lab_sander_IPF.case_group_gemini_weekly_ss_umls_code where before_after_dx = 'before'
# MAGIC union all
# MAGIC select 'control' as group,count(*),count(distinct person_id) from wei_lab_sander_IPF.control_group_gemini_weekly_ss_umls_code where before_after_dx = 'before'
...
for group in group_list:
    for model in model_list:
        ...
        # Step 2: Generate symptom co-occurrence pairs
        # (within the same person and the same week)
        df_pairs = (
            df_active.alias("a")
            .join(
                df_active.alias("b"),
        ...
        # Step 3: Count co-occurrence frequency
        # and number of distinct patients with this symptom pair
        df_pair_counts = (
...
# MAGIC %sql
# MAGIC select * from wei_lab_sander_IPF.case_gemini_symptom_pair_cooccurrence order by distinct_person_count desc
...
for group in group_list:
    for model in model_list:
        ...
        top_pairs_list = [
            (r["symptom_1"], r["symptom_2"])
            for r in df_top.collect()
        ]
        ...
        df_week_indexed = (
            ...
            .withColumn(
                "week_index",
                (F.row_number().over(w_idx) - 1).cast("int")
            )
        )
        ...
        # ----------------------------
        # 3. Build joint_state for each pair,
        ...
        for s1, s2 in top_pairs_list:
            ...
            df_pair_week = (
                ...
                .filter(F.col("next_state").isNotNull())
            )
            ...
            df_joined = (
                ...
                .agg(
                    F.collect_set("nxt.umls_name")
                        .alias("other_symptoms_in_next_week"),
                    F.collect_set("nxt.symptom_with_severity")
                        .alias("other_symptoms_with_severity")
                )
                .withColumn(
                    "symptom_pair",
                    F.lit(f"{s1}|{s2}")
...
# MAGIC %sql
# MAGIC select 'case' as group, count(*),count(distinct person_id) from wei_lab_sander_IPF.case_gemini_pfsa_pair_with_symptoms where joint_state = '1_1'
# MAGIC union all
# MAGIC select 'control' as group, count(*),count(distinct person_id) from wei_lab_sander_IPF.control_gemini_pfsa_pair_with_symptoms where joint_state = '1_1'
...
# MAGIC %sql
# MAGIC select CUI,UMLS_name as umls_name,tfidf as importance_score from wei_lab_sander_mlflow.llm_ro_tf_idf_results 
# MAGIC                         where disease='Idiopathic Pulmonary Fibrosis' order by importance_score desc
...
for group in group_list:
    for model in model_list:
        ...
        # Load importance scores (TF-IDF from LLM results)
        imp_df = spark.sql(f"""
            SELECT
                CUI,
        ...
        df_score.select(
            ...
            f"wei_lab_sander_IPF.{group}_{model}_patient_symptom_weight_TFIDF"
        )
...
for group in group_list:
    for model in model_list:
        ...
        # Load patient-level final weights (TF-IDF version),
        # and prepare renamed columns for s1 / s2
        ...
        # 6️⃣ Extract all symptoms (s1 / s2) with their weights
        ...
        patient_symptom_weight_df.write.mode("overwrite").saveAsTable(
            f"wei_lab_sander_IPF.{group}_{model}_pfsa_pair_pt_total_weight_TFIDF"
        )
...
# MAGIC %sql
# MAGIC select * from wei_lab_sander_IPF.case_o3_pfsa_pair_pt_total_weight_TFIDF order by total_symptom_weight desc
...
for group in group_list:
    for model in model_list:
        ...
        # (only pre-diagnosis records with joint_state = '1_1')
        pair_df = spark.table(
            f"wei_lab_sander_IPF.{group}_{model}_pfsa_pair_with_symptoms"
        )
...
def stratified_oob_bootstrap(
    ...
    for _ in range(n_bootstrap):
        ...
        # --- cutoff determined on training only ---
...
for model in models:
    for version in versions:
        ...
        print(f"\nRunning model: {label_map[model]} (TF-IDF)")
        ...
        # -----------------------------
        # Stratified OOB Bootstrap (paper-grade)
        # -----------------------------
        boot = stratified_oob_bootstrap(pdf, n_bootstrap=100)
        bootstrap_summary[f"{model}{version}"] = boot
        ...
        roc_data.append((f"{model}{version}", fpr, tpr))
...
for i, (key, fpr, tpr) in enumerate(roc_data):
    base_model = key.replace("_TFIDF", "")
    plt.plot(
        fpr, tpr,
...
plt.title("ROC Curve Comparison by LLM Source (TF-IDF, Stratified OOB Bootstrap)")
...
# =====================================================
label_map = {
    "ro_tfidf": "Rarediseases.org",
    "ds_tfidf": "DeepSeek-R1",
    "o3_tfidf": "OpenAI o3",
    "claude_tfidf": "Claude-Sonnet-4",
    "gemini_tfidf": "Gemini-2.5 Pro"
}

# =====================================================
# Use bootstrap_summary ONLY (paper-grade source)
# =====================================================
...
# =====================================================
# Optional: summary table (appendix / sanity check)
# =====================================================
df_summary = (
    pd.DataFrame({
        "Model": [label_map.get(m.lower(), m) for m in models],
        "AUROC": [round(v, 3) for v in auc_scores],
        "F1": [round(v, 3) for v in f1_scores],
        "AUROC 95% CI": [f"[{auc_ci[m][0]:.3f}, {auc_ci[m][1]:.3f}]" for m in models],
        "F1 95% CI": [f"[{f1_ci[m][0]:.3f}, {f1_ci[m][1]:.3f}]" for m in models],
    })
    .sort_values(by="AUROC", ascending=False)
)

print("\n📋 Model performance summary (bootstrap mean ± 95% CI):")
print(df_summary.to_string(index=False))

# =====================================================
# Plot: AUROC vs F1 with bootstrap CI
# =====================================================
...
for i, model in enumerate(models):
    ...
    # --- Bootstrap confidence intervals (draw first) ---
    plt.errorbar(
        auc_scores[i],
        f1_scores[i],
        xerr=[
...
plt.xlabel("AUROC")
plt.ylabel("F1 Score")
plt.title("Overall Performance: AUROC vs F1 Across Models (TF-IDF, Bootstrap)")

plt.grid(True, linestyle="--", alpha=0.35, zorder=1)

# ⬇️ Start both axes from zero
plt.xlim(0.5, max(auc_scores) + 0.03)
plt.ylim(0.3, max(f1_scores) + 0.05)
